...
function updatePackageLockJson(newVersion: string): void {
    ...
    const packageLock: PackageLock = JSON.parse(fs.readFileSync(packageLockPath, 'utf8'));
...
function updateScriptsReadme(newVersion: string): void {
...
function updateChangelog(version: string, type: VersionType): void {
...
function validateVersionConsistency(expectedVersion: string): boolean {
    ...
    try {
        ...
        issues.push(`Cannot read package.json: ${errorMessage}`);
    }
...
function updateAllVersionFiles(newVersion: string): void {
    ...
    for (const versionFile of VERSION_FILES) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            log(`❌ Failed to update ${versionFile.file}: ${errorMessage}`, 'red');
        }
    }
}
...
function createGitTag(version: string): void {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        log(`⚠️  Git operations failed: ${errorMessage}`, 'yellow');
    }
}
