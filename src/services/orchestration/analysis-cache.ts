...
export class AnalysisCache extends EventEmitter {
  ...
  public async getOrAnalyze<T>(
    ...
    // Store in cache
    await this.store(fullKey, result, options.ttl);
    this.recordMetric('cache_store', 1);
    this.recordMetric('analysis_time', analysisTime);
  ...
  private cleanupExpiredEntries(): void {
    ...
    if (expiredKeys.length > 0) {
      ...
      this.emit('entriesExpired', expiredKeys);
    }
  }

  private recordMetric(name: string, value: number): void {
    // Simplified metric recording to avoid interface issues
    try {
      this.performanceMonitor.recordMetric(name, {
        timestamp: Date.now(),
        value,
        unit: 'count',
      ...
      // Fallback - just continue if metric recording fails
