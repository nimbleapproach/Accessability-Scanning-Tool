...
export class PdfOrchestrator {
            ...
            try {
                for (const audience of audiences) {
                    const result = await this.generateSinglePdfReport(
                        ...
                        audience,
                        filename,
                        { format, includeBackground, margins }
                    );
                ...
                this.errorHandler.logSuccess(`Generated ${results.length}/${audiences.length} PDF reports`);
                return results;
            } finally {
                await browser.close();
            }
    ...
    async cleanup(): Promise<void> {
        try {
            const tempFiles = require('fs').readdirSync(process.cwd())
            ...
            this.errorHandler.logWarning('Failed to cleanup temporary files', { error });
        }
    }
