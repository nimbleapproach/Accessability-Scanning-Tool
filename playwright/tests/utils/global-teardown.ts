...
async function clearTemporaryFiles(): Promise<void> {
    ...
    try {
        ...
        if (process.platform !== 'win32') {
            ...
            await execAsync('find /tmp -name "*playwright*" -type d -exec rm -rf {} + 2>/dev/null || true');
        }
...
async function verifyPlaywrightCleanup(): Promise<void> {
    try {
        ...
        const { stdout } = await execAsync('ps aux | grep -E "playwright.*chrome|headless_shell.*playwright" | grep -v grep || true');
