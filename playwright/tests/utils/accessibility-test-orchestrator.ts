...
import { AxeTestRunner } from './runners/axe-test-runner';
import { Pa11yTestRunner } from './runners/pa11y-test-runner';
import { WaveTestRunner } from './runners/wave-test-runner';
import { TenonTestRunner } from './runners/tenon-test-runner';
import {
  MultiToolResults,
  ProcessedViolation,
  ViolationProcessor,
} from './processors/violation-processor';
import { PageAnalysisResult, PageAnalyzer } from './analyzers/page-analyzer';
import { ConfigurationService } from './services/configuration-service';
import { ErrorHandlerService, ServiceResult } from './services/error-handler-service';
import { FileOperationsService } from './services/file-operations-service';
import { TestSetupUtil } from './test-setup-util';
...
export class AccessibilityTestOrchestrator {
  ...
  private waveRunner: WaveTestRunner;
  private tenonRunner: TenonTestRunner;
  ...
  constructor(private page: Page) {
    ...
    this.waveRunner = new WaveTestRunner(page);
    this.tenonRunner = new TenonTestRunner(page);
  ...
  async runComprehensiveAnalysis(
    ...
    return this.errorHandler.executeWithErrorHandling(async () => {
      ...
      const [axeResult, pa11yResult, waveResult, tenonResult] = await Promise.all([
        this.axeRunner.runDevToolsAnalysis(),
        this.pa11yRunner.runAnalysis(),
        this.waveRunner.runComprehensiveAnalysis(),
        this.tenonRunner.runWCAGAAAnalysis(),
      ]);

      // Prepare results for processing
      const results: MultiToolResults = {
        axe: this.errorHandler.isSuccess(axeResult) ? axeResult.data : null,
        pa11y: this.errorHandler.isSuccess(pa11yResult) ? pa11yResult.data : null,
        lighthouse: null, // Can be added later if needed
        wave: this.errorHandler.isSuccess(waveResult) ? waveResult.data : null,
        tenon: this.errorHandler.isSuccess(tenonResult) ? tenonResult.data : null,
      };
      ...
      this.errorHandler.logSuccess(
        `Comprehensive analysis completed: ${processedResult.data.length} violations found`
      );
      ...
      if (options.colorContrast) {
        ...
        if (this.errorHandler.isSuccess(result)) {
          ...
          if (this.errorHandler.isSuccess(processed)) {
            allViolations.push(...processed.data);
          }
        }
      }
  ...
  async generateAccessibilityReport(
    ...
    return this.errorHandler.executeWithErrorHandling(async () => {
      ...
      if (this.errorHandler.isError(violationsResult)) {
        throw new Error(violationsResult.error);
      }

      if (this.errorHandler.isError(pageAnalysisResult)) {
        throw new Error(pageAnalysisResult.error);
      }
  ...
  aggregateReports(reports: AccessibilityReport[]): SiteWideAccessibilityReport {
    ...
    const testSuite = reports.length > 0 ? reports[0].testSuite : 'Unknown';
