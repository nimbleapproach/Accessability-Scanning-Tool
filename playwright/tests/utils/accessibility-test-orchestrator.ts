...
import { ViolationProcessor, ProcessedViolation, MultiToolResults } from './processors/violation-processor';
import { PageAnalyzer, PageAnalysisResult } from './analyzers/page-analyzer';
...
export interface SiteWideAccessibilityReport {
    ...
    violationsByType: Record<string, {
        count: number;
        pages: string[];
        impact: string;
        totalOccurrences: number;
...
export class AccessibilityTestOrchestrator {
    ...
    async runComprehensiveAnalysis(
        ...
        return this.errorHandler.executeWithErrorHandling(async () => {
            ...
            this.errorHandler.logSuccess(`Comprehensive analysis completed: ${processedResult.data.length} violations found`);
            ...
            if (options.colorContrast) {
                ...
                if (this.errorHandler.isSuccess(result)) {
                    const processed = await this.violationProcessor.processMultiToolViolations(
                        { axe: result.data, pa11y: null, lighthouse: null }
                    );
    ...
    aggregateReports(reports: AccessibilityReport[]): SiteWideAccessibilityReport {
        ...
        const summary = {
            ...
            compliancePercentage: reports.length > 0 ?
                ((reports.length - reports.filter(r => r.violations.length > 0).length) / reports.length) * 100 : 0,
